...
return array(
    ...
    "admin.setting.passwordErrorLockDesc" => "Если пароль неверен 5 раз подряд, учетная запись блокируется на 30 секунд и вход в систему не разрешается.После открытия это может эффективно предотвратить взлом пароля методом перебора;",
...
<li> Диапазон IP-адресов: IP соответствует диапазону; например: 192.168.1.1-192.168.200.255</li>",
    ...
    "admin.loginCheck.desc"          => "<h5 class=\"mb-15\"> <b>Инструкции по контролю ограничения входа пользователя (ограничения IP и устройства):</b></h5><li> Обнаруживать последовательно в соответствии с приоритетом правила; пользователь, указанный правилом, включает в себя текущего пользователя, вошедшего в систему; правило определяется как результат</li><li> Рекомендуется размещать группы пользователей и пользователей отдела на оборотной стороне, а параметры пользователя указывать на передней панели; (перетащите и отпустите, чтобы изменить порядок)</li>",
    ...
    "admin.setting.phpExt"           => "Расширение PHP",
    ...
    "admin.setting.ifSaveDb"         => "Переключение базы данных заключается в импорте данных текущей системной базы данных в новую базу данных и установке новой базы данных в качестве системной базы данных. Вы уверены, что хотите выполнить?",
    ...
    "admin.share.disAsReport"        => "На этот файл пожаловались пользователи, и доступ к нему заблокирован",
