...
<li>IP区间段: ip在区间内则匹配;例如:192.168.1.1-192.168.200.255</li>",
    ...
    "admin.loginCheck.desc"          => "<h5 class=\"mb-15\"><b>用户登陆限制管控说明(ip及设备限制):</b></h5>           <li>按规则优先级顺序依次检测; 规则指定用户包含当前登陆用户;则以该规则判定作为结果</li>        <li>建议将用户组,部门用户范围大的放在后面, 指定用户设置的放在前面;(可拖拽调整顺序)</li>",
    ...
    "admin.setting.ifSaveDb"         => "数据库切换，是将当前系统库数据导入至新的数据库，并将新的库设置为系统数据库。确定要执行吗？",
    ...
    "ERROR_IP_NOT_ALLOW"             => "您当前IP或访问设备不允许登陆,请联系管理员!",
    ...
    "admin.share.disAsReport"        => "该文件遭到用户举报，已被禁止分享",
