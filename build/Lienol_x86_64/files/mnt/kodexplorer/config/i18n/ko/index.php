...
return array(
    ...
    "admin.setting.passwordErrorLockDesc" => "비밀번호가 5 회 연속 부정확하면 30 초 동안 계정이 잠기고 로그인이 허용되지 않으며, 개봉 후 무차별 암호 해독을 효과적으로 방지 할 수 있습니다.",
...
<li> IP 범위 : ip는 범위 내에서 일치합니다 (예 : 192.168.1.1-192.168.200.255).</li>",
    ...
    "admin.loginCheck.desc"          => "<h5 class=\"mb-15\"> <b>사용자 로그인 제한 제어 지침 (ip 및 장치 제한) :</b></h5><li> 규칙의 우선 순위에 따라 순서대로 탐지합니다. 규칙에 지정된 사용자에는 현재 로그인 한 사용자가 포함됩니다. 결과에 따라 규칙이 결정됩니다.</li><li> 사용자 그룹 및 부서별 사용자는 뒷면에 배치하고 사용자 설정은 전면에 지정하는 것이 좋습니다. (드래그 앤 드롭으로 순서를 조정)</li>",
    ...
    "admin.setting.ifSaveDb"         => "데이터베이스 전환은 현재 시스템 데이터베이스 데이터를 새 데이터베이스로 가져오고 새 데이터베이스를 시스템 데이터베이스로 설정하는 것입니다. 실행 하시겠습니까?",
    ...
    "admin.share.disAsReport"        => "이 파일은 사용자가 신고했으며 공유가 차단되었습니다.",
